...
/*	
	$scope.searchResult = $scope.empData = $scope.getEmpData('employeeData-depth5.json');
	$scope.renderOptions = $scope.getRenderOptions('renderOptions.json');
...
*/
	...
	$scope.getEmpData = function(j) {
		$http.get(j).success(function(data) {
			return data.employeeData;
	})};

	$scope.getRenderOptions = function(r) {
